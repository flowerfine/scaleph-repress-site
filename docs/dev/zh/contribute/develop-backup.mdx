...
开发者需安装如下程序，参考：[准备环境](../quick-start/compile/index.md)

- [JDK](https://adoptium.net/zh-CN/temurin/archive/)。下载并安装 open JDK 17。
- [Node](https://nodejs.org/en/download/releases/)。下载并安装 Node 16。
- [Docker](https://docs.docker.com/get-docker/)。下载并安装 Docker。已有 Docker 环境时，使用 `docker version` 和 `docker compose version` 检查对应的版本，避免版本过低造成启动失败。
...
## 只下载默认分支代码，避免下载所有分支、tag 和历史提交信息，导致 clone 时间过久
